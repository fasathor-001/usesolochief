...
export default async function AdminSystemPage() {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr minmax(240px, 30%)', gap: 16, alignItems: 'start' }}>
          ...
          <div className="sc-card" style={{ padding: '18px 20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 10 }}>
              {Object.entries(s.tableCounts).map(([table, count]) => (
                <div key={table} style={{ padding: '10px 12px', borderRadius: 6, border: '0.5px solid var(--sc-border)', background: 'var(--sc-bg)' }}>
        ...
        <div className="sc-card" style={{ padding: '18px 20px' }}>
          ...
          <div style={{ marginBottom: 12, paddingBottom: 12, borderBottom: '0.5px solid var(--sc-border)' }}>
            <p style={{ fontSize: 11, color: 'var(--sc-muted)', marginBottom: 4 }}>Environment</p>
            <span style={envBadgeStyle}>{s.environment}</span>
          </div>
          <div style={{ marginBottom: 12, paddingBottom: 12, borderBottom: '0.5px solid var(--sc-border)' }}>
            <p style={{ fontSize: 11, color: 'var(--sc-muted)', marginBottom: 4 }}>App URL</p>
            <code style={{ fontSize: 11, color: 'var(--sc-text)', wordBreak: 'break-all' }}>{s.appUrl}</code>
          </div>
